...
mod my_object {
    ...
    impl qobject::Rot {
        ...
        pub fn encrypt(&self, plain: &QString) -> QString {
            ...
            let bytes_result = rot(nrot::Mode::Encrypt, plain, rotation);
            let mut secret = format!("{}", String::from_utf8_lossy(&bytes_result));

            if plain.len() == 1 {
                let byte_result = rot_letter(nrot::Mode::Encrypt, plain[0], rotation);
        ...
        pub fn decrypt(&self, secret: &QString) -> QString {
            ...
            let bytes_result = rot(nrot::Mode::Decrypt, secret, rotation);
            let mut plain = format!("{}", String::from_utf8_lossy(&bytes_result));

            if secret.len() == 1 {
                let byte_result = rot_letter(nrot::Mode::Decrypt, secret[0], rotation);
